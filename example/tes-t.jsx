...
class WList extends Component {
  ...
  getDirectleaderList = (payload) => {
    ...
    getListData(payload).then(res => {
      ...
      (array || []).forEach(t => { t.value = t.employeeId; t.title = t.employeeName; t.key = randomstring.generate(5) })
  ...
  render() {
    ...
    const content = (getRecord) => {
      const dimission = (
        <Authorized authority={["hr_person_resignation"]} noMatch={null}>
          <i
            title="离职"
      ...
      const transfer = (
        <Authorized authority={["hr_person_transferLevel"]} noMatch={null}>
          <i
            title="调岗"
      ...
      const jobRelocation = (
        <Authorized authority={["hr_person_transferPosition"]} noMatch={null}>
          <i
            title="调职级"
      ...
      const partTimeJob = (
        <Authorized authority={["hr_person_concurrentPosition"]} noMatch={null}>
          <i
            title="兼岗"
      ...
      const regular = (
        <Authorized authority={["hr_person_regular"]} noMatch={null}>
          <i
            title="转正式"
      ...
      const againIn = (
        <Authorized authority={["hr_person_reentry"]} noMatch={null}>
          <i
            title="再入职"
      ...
      const formalOpt = (
        <span className={styles.formalOpt}>
          { dimission }
          { transfer }
          { jobRelocation }
          { partTimeJob }
      ...
      render: (text, getRecord) => {
        return (
          <Authorized
            ...
            noMatch={<span className={styles.optsAction}>{text}</span>}
          >
    ...
    const columnsJob = [{
      ...
      render: (text, recordData) => {
        return (
          <Popconfirm title="确定要取消兼岗吗?" onConfirm={() => this.cancelPartTimeJob(recordData)}>
            <span><a className={styles.optsAction}>取消兼岗</a></span>
          </Popconfirm>
        )
      },
    }]
    ...
    const getFormByType = (formType) => {
      ...
      if (formType === 'partTimeJob') {
        return addPartTimeJob ? <PartTimeJob getIdOfPositionInfo={this.getIdOfPositionInfo} form={form} getFieldDecorator={form.getFieldDecorator} />
          : (
            <div style={{ paddingBottom: 50 }}>
              <Button style={{ marginBottom: 10 }} type="primary" onClick={this.addPartTimeJob}>新增兼岗</Button>
    ...
    return (
      <div>
        ...
        <HRPagination currentPage={currentPage} pageSize={pageSize} listTotal={newData.totalCount} onChange={(page, size) => {this.paginationChange(page, size)}} />
        <Modal
          visible={visible}
          ...
          footer={!addPartTimeJob && modalType === 'partTimeJob' ? null : [
            <Button className={styles.submitModal} key="submit" type="primary" onClick={this.submitForm.bind(this)}>
              提交
            </Button>,
            <Button className={styles.submitModal} style={{ marginLeft: 15 }} key="back" onClick={this.handleCancel}>关闭</Button>,
          ]}
        >
          {
            record ? (
              <Row className={styles.userInfoTitle} type="flex" justify="space-between">
                <Col span={3} className={styles.userName}>{record.employeeName}</Col>
                <Col span={7}>职业生涯： {record.careerName}</Col>
                <Col span={7}>职级： {record.jobLevelName}</Col>
                <Col span={7}>{modalType === 'againIn' ? `离职日期：${moment(leaveDate).format(dateFormat)}` : `入职日期：${moment(record.entryDate).format(dateFormat)}`} </Col>
                <Col offset={3} span={7}>部门：{record.deptName}</Col>
                <Col span={7}>岗位：{record.positionName}</Col>
                <Col span={7} />
              </Row>) : null
          }
